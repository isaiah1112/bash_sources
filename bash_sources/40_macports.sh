...
function portclean() {
    sudo port clean -f --all installed; # Clean all active/installed ports 
    while true; do
        if ! sudo port -f uninstall inactive; then break; fi
        if ! sudo port uninstall leaves; then break; fi # Uninstall orphaned dependancies
    done
}
